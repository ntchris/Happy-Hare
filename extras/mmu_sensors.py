...
class SyncTensionSensorAdj:
    ...
    # we can use 90% and 110% of rotation_dist as initial upper and lower limit, it's not important. just an initial value
    INITIAL_ADJ = 0.10
    DEFAULT_ROTA_DIST = 22.7316868
    def __init__(self):
        ...
        self.rotation_dist = SyncTensionSensorAdj.DEFAULT_ROTA_DIST	 # doesn't really matetr

    def init_for_next_print(self, rota_dist=DEFAULT_ROTA_DIST):
    ...
    def update_sync_rotation_dist(self, state: float) -> float:
        if SyncTensionSensorAdj.DEBUG:
           self.log_debug(f"update_sync_rotation_dist working {state=}")
        ...
        if self.is_sync_tension_compressed(state):
            ...
            if self.stayed_in_tension_count > SyncTensionSensorAdj.ALLOW_STAY_IN_TENSION_STATE_MAX:
                ...
                new_val = self.bad_rotation_dist_expanded * (1 + SyncTensionSensorAdj.ADJUST_WHEN_STAY_TENSION_TOO_LONG)
                ...
                f"expanded value larger {SyncTensionSensorAdj.ADJUST_WHEN_STAY_TENSION_TOO_LONG * 100}%, now {new_val:.6f}(was {self.bad_rotation_dist_expanded:.6f})")
                self.bad_rotation_dist_expanded = new_val
                self.stayed_in_tension_count = 0

            new_rota_dist = self.bad_rotation_dist_expanded
            ...
            if self.stayed_in_tension_count > SyncTensionSensorAdj.ALLOW_STAY_IN_TENSION_STATE_MAX:
                ...
                new_val = self.bad_rotation_dist_compressed * (1 - SyncTensionSensorAdj.ADJUST_WHEN_STAY_TENSION_TOO_LONG)
                # just to make sure the expand value can make more filament (so to ensure to reach the expanded state)
                self.log_debug(f"stayed too long in expanded state? {self.stayed_in_tension_count=}(too long time), make known bad " 
                f"compressed value smaller {SyncTensionSensorAdj.ADJUST_WHEN_STAY_TENSION_TOO_LONG * 100}%, now {new_val}(was {self.bad_rotation_dist_compressed})")
                self.stayed_in_tension_count = 0
                self.bad_rotation_dist_compressed = new_val

            new_rota_dist = self.bad_rotation_dist_compressed
    ...
    def set_init_bad_values(self) -> None:
        ...
        if not self.bad_rotation_dist_compressed:
            ...
            self.bad_rotation_dist_compressed = self.rotation_dist * (1.0 - SyncTensionSensorAdj.INITIAL_ADJ)

        if not self.bad_rotation_dist_expanded:
            # this value will cause expanded
            self.bad_rotation_dist_expanded = self.rotation_dist * (1.0 + SyncTensionSensorAdj.INITIAL_ADJ)
        return
    ...
    def cal_rotation_dist_by_tension_value(self, compressed:float, expanded:float) -> float:
        ...
        self.log_always(
            f"using last compressed {compressed:.6f} and last expanded {expanded:.6f} to avg: have new better rotation_dist:{rotation_dist:.6f}")
    ...
    def get_rotation_dist_on_state_change(self, last_state: float, state: float) -> float:
        ...
        if SyncTensionSensorAdj.is_sync_tension_compressed(state):
            ...
            self.log_debug(f"tension state changed to compressed(from neutral), record current rota_dist as new bad compressed val:{self.rotation_dist:.6f}(current rota dist)")

            if math.isclose(self.rotation_dist, self.bad_rotation_dist_expanded):
                adjusted_bad_rotation_dist_expanded = self.bad_rotation_dist_expanded * ( 1.0 + SyncTensionSensorAdj.ADJUST_WHEN_ALREADY_PERFECT)
                self.log_debug(f"already have the perfect rota dist, adjust a little bit to make to move to expanded direction {self.bad_rotation_dist_expanded:.6f}->{adjusted_bad_rotation_dist_expanded:.6f}")
                self.bad_rotation_dist_expanded = adjusted_bad_rotation_dist_expanded
            # immediately return the bad value of other dir
            self.rotation_dist = self.bad_rotation_dist_expanded
            self.log_debug(f"for compressed state, return new rota_dist {self.rotation_dist:6f}")
            return self.rotation_dist
            ...
            self.log_debug(f"tension state changed to expanded(from neutral), record current rota_dist as new bad expanded val:{self.rotation_dist:.6f}(current rota dist)")

            if math.isclose(self.rotation_dist, self.bad_rotation_dist_compressed):
                adjusted_bad_rotation_dist_compressed = self.bad_rotation_dist_compressed * ( 1.0 - SyncTensionSensorAdj.ADJUST_WHEN_ALREADY_PERFECT)
                self.log_debug(f"already have the perfect rota dist, adjust a little bit to make to move to compressed direction {self.bad_rotation_dist_compressed:.6f}->{adjusted_bad_rotation_dist_compressed:.6f}")
                self.bad_rotation_dist_compressed = adjusted_bad_rotation_dist_compressed

            self.rotation_dist = self.bad_rotation_dist_compressed
            self.log_debug(f"for expanded state, return new rota_dist {self.rotation_dist:6f}")
            return self.rotation_dist
        ...
        if self.bad_rotation_dist_compressed > self.bad_rotation_dist_expanded:
            self.log_error(f"impossible! why bad compressed rota_dist is larger than bad expanded rota ?? {self.bad_rotation_dist_compressed}>{self.bad_rotation_dist_expanded}, just swap them")
            ...
            self.log_error(f"after swap, now compressed rota dist:{self.bad_rotation_dist_compressed} < expanded rota dist:{self.bad_rotation_dist_expanded}")
        ...
        if SyncTensionSensorAdj.is_sync_tension_neutral(state):
            ...
            #if math.isclose(self.bad_rotation_dist_compressed, self.bad_rotation_dist_expanded):
            #    pass
            #else:
            ...
            if math.isclose(rotation_dist, self.rotation_dist):
                ...
                self.log_debug(f"already have the perfect rotation dist {rotation_dist:.6f}..." )
